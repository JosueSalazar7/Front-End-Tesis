...
const TablaViajesCompartidos = () => {
    const navigate = useNavigate();
    const [viajesCompartidos, setViajesCompartidos] = useState([]);

    const listarViajesCompartidos = async () => {
        try {
            ...
            setViajesCompartidos(respuesta.data.boletos);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    useEffect(() => {
        listarViajesCompartidos();
    }, []);

    const handleDelete = async (id) => {
        try {
            const confirmar = window.confirm(
                "Vas a eliminar el siguiente viaje compartido"
            );
            if (confirmar) {
                const token = localStorage.getItem("token");
    ...
    const data = React.useMemo(() => viajesCompartidos, [viajesCompartidos]);
    ...
    const columns = React.useMemo(
        () => [
            ...
            {
                Header: "Apellido del Pasajero",
                accessor: "apellido",
            },
            {
                Header: "Ciudad de Origen",
                    ...
                    <div className="flex justify-between items-center mb-4">
                        <div className="relative flex-1">
                            <input
                                ...
                                placeholder="Busca por nombre de pasajero..."
...
export default TablaViajesCompartidos;
