...
const TablaViajesPrivadosPendientes = () => {
    ...
    const listarViajesPendientes = async () => {
        try {
            ...
            const url = `${import.meta.env.VITE_BACKEND_URL}/admin/viajesPriv`;
            const options = {
                headers: {
            ...
            if (respuesta.status === 200) {
                ...
                console.error("Error al obtener los viajes privados pendientes:", respuesta.data.error);
            }
        } catch (error) {
            console.error("Error de red al obtener los viajes privados pendientes:", error);
        }
    };
    ...
    const handleDelete = async (id) => {
        try {
            const confirmar = window.confirm(
                "¿Estás seguro de que deseas eliminar este viaje privado pendiente?"
            );
            if (confirmar) {
                const token = localStorage.getItem("token");
                const url = `${import.meta.env.VITE_BACKEND_URL}/admin/eliminarPriv/${id}`;
                const headers = {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                };
                await axios.delete(url, { headers });
                // Refrescar la lista de viajes privados pendientes después de eliminar
                listarViajesPendientes();
            }
        } catch (error) {
            console.log(error);
...
export default TablaViajesPrivadosPendientes;
